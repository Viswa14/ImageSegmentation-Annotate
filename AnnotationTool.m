...
% Last Modified by GUIDE v2.5 03-Oct-2016 14:39:17
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
 value = get(handles.buttongroup,'SelectedObject');
 value_string = get(value,'String');
 path = get(handles.edit1,'String');
 if isempty(path)
     msgbox('Please Specify the path to extract files');
 else
    closereq;
    setappdata(0,'type',value_string);
    setappdata(0,'path',path);
    AnnotationToolBoundingBox;
 end
